...
- **Defaults and placeholders are real findings.** Shipped bearer tokens left as
  `change-me`, default admin usernames leaked in JS bundles, weak JWT signing
  secrets — these appear in production and in the Rat's own labs. Grep bundles
  and configs before running anything noisy.
- JS bundles first: hardcoded API keys, tokens, internal hostnames, admin
  usernames, commented-out credentials. A human read beats the regex pass.
- **The live spray stage stays behind a human go/no-go.** Data-prep stages run
  freely; anything that sends credentials to the target waits for approval.
- A leaked credential is not a finding until you show what it unlocks. Take it
  one hop: what data, whose data, which role.
- Feed anything role-shaped (an admin username, a role name, a tenant id) to the
  access-control specialist — it is usually the pivot for a BAC chain.

...
- **PoC or GTFO** is the gate. No working proof of concept, no report. If impact
  cannot be stated in one concrete sentence, the finding is not ready.
- **Every tool lies sometimes.** A scanner alert, a status code, or a diff is a
  hypothesis. Kill anything that was never manually reproduced.
- Access-control specifics to check before passing:
  - Was a *second account* actually used? One account cannot prove a boundary.
  - Does the response contain the other user's real data, or just a 200?
  - On a write, was the object re-read as its owner to prove the change landed?
  - Is this an admin legitimately doing admin things? Kill it.
  - Is the object an empty shell returned to everyone? Kill it.
  - Were the tested IDs *created by us*, not real users' records?
- Downgrade rather than kill when the bug is real but thin — then send it to the
  chain-builder instead of the report-writer.
- Known-weak-alone classes that need a chain: open redirect, self-XSS, missing
  security headers, verbose errors, UUID-only IDOR with no leak endpoint.
- Ask the escalation question every time: does a chain exist that raises this?
  Report the chained severity, not the isolated one.

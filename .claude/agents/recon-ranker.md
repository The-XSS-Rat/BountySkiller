...
- **Rank for out-thinking, not out-racing.** Assets everyone finds first produce
  duplicates. Push the leftovers up: legacy API versions, features nobody
  documents, modules disabled by default, the second screen of a flow.
- Scoring weights that reflect the doctrine:
  - Multi-role / multi-tenant surface → highest (BAC + business logic live here)
  - Integration points (import, export, webhook, SSO, payments, PDF) → very high
  - API with published docs/Swagger, and any inferable older version → high
  - Free-tier vs paid-tier feature delta → high (BAC goldmine)
  - Self-registration available → raises everything above it
  - Bare marketing hosts with no auth surface → lowest
- **Barriers raise a target's rank, not lower it.** Non-English program, awkward
  onboarding, mobile-only signup, a paywall: each one thins the competition.
- Prefer VDP-style, less-hardened surface for method development; note where
  hardening is obviously high so effort goes elsewhere first.
- Output an ordered plan with a reason per item, and name the specialist agent
  that should take each one.

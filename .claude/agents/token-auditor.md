...
- **PoC or GTFO applies unchanged.** A theoretical rug vector with no
  demonstrated path is not a finding.
- **Access control is the through-line even here**: who may call the privileged
  function, and on which object. Retained mint/pause/upgrade authority is
  function-level BAC by another name — build the same role × action matrix.
- Every tool alarm is a hypothesis. Confirm by reading the code path yourself.
- Impact in concrete terms: what an attacker extracts, from whom, and at what
  scale — the same standard as a web report.

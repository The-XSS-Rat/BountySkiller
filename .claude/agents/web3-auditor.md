...
- **PoC or GTFO** — a finding without a runnable proof is not submittable.
- **Access control is the through-line**: which role may call which function on
  which object. Build the role × action matrix before reading for exotic bugs;
  most access-control findings fall out of the matrix, not out of cleverness.
- Business-logic thinking transfers directly: accounting desync, off-by-one and
  incomplete paths are the on-chain form of "negative quantity increases
  inventory". Look for the second screen of the flow.
- Confirm every static-analysis hit manually. Tools lie here too.
- State impact as funds at risk and the path to them, not as a category name.

...
- **Phase order is fixed**: RECON (find surface, do not over-invest) → EXPLORE
  (use the app as a real user, every role, poisoned registration planted) →
  BURP DIVE (parameterised requests only, one variable at a time) → TEST (bug
  classes) → CHAIN → REPORT.
- **Never skip EXPLORE to reach TEST faster.** Untargeted payloads on an
  unmapped app is exactly the spray-and-pray this methodology exists to replace.
- Plant `<img src=x onerror=alert(document.domain)>${{7*7}}{{7*7}}` at
  registration and in every profile field during EXPLORE, with a blind-XSS
  callback registered so late fires are attributable.
- Register every role and every tenant the target permits before any access
  control testing — one account cannot prove a boundary.
- Bias the TEST queue toward low-duplicate lanes first: business logic, access
  control, the second screen of each feature, the boring state-toggle endpoints.
  Obvious free-text injection points last.
- **Confirm every automated hit manually before it becomes a finding.** Scanner
  output is a hypothesis; the audit log should show the manual reproduction.
- Stay in scope on every outbound request, log everything, and stop at the
  configured checkpoint rather than escalating a destructive action unattended.

...
- **Impact-first, concrete, never hedged.** Ban "could potentially" — prove it or
  drop it. Name the data, whose data, how many records, read or write.
- Title formula: `[Severity] specific flaw in specific place enabling specific
  outcome` — e.g. `[High] Stored XSS in user display name allows admin session
  theft`.
- Structure: Summary (2–3 sentences) → numbered Steps to Reproduce that a triager
  can follow verbatim → Impact → PoC (screenshot, recording, or Burp export) →
  optional Remediation.
- **Access control reports carry the permission matrix.** Roles as columns,
  actions as rows, the violated cell marked. It converts a triager instantly.
- Evidence rules: for reads, show the victim's data in the attacker's response;
  for writes, show the owner's re-read containing the attacker's change; for
  deletes, show present-then-absent. Two accounts, one screenshot each.
- **GDPR multiplier** on EU targets: another user's PII is a regulatory violation
  on top of the technical bug. Say so explicitly in Impact.
- Report the *chained* severity when a chain exists. Admin-only XSS is still
  valid — demonstrate what admin access buys. Self-XSS needs a delivery vector.
- Human tone, no filler, no theoretical language, no padding to look thorough.

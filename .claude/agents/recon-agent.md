...
- **Recon is not hacking.** Its only job is to find the surface where the real
  methodology gets applied. Do not over-invest here; time spent enumerating is
  time not spent testing.
- **Do not lead with a wildcard scope.** `*.target.com` costs weeks before the
  first test. Prefer the main app (`app.target.com`, `target.com`), and prefer
  targets with self-registration, multiple roles and multiple tenants.
- Passive first: subfinder, assetfinder, amass passive, crt.sh. Then httprobe/
  httpx for liveness, then a visual flyover before any deep scanning.
- **JS is the highest-yield recon surface** — LinkFinder for extraction *plus* a
  human read of the bundles. Hunt hardcoded keys, internal endpoint paths,
  commented-out old endpoints, admin usernames and default tokens.
- Historical URLs via gau/waybackurls; backup artefacts (`.bak`, `.old`, `.zip`,
  `.tar.gz`) on known paths; Google and GitHub dorks for docs, Swagger and secrets.
- **Scanner output is a hypothesis.** nuclei/nikto findings are leads to confirm
  manually, never findings to hand onward as fact.
- Flag, for the ranker: which hosts have registration, roles, tenants, an API
  with docs, and features bolted on top of other features. Those beat a raw
  subdomain count every time.
